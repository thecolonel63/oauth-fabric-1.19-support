...
public class ProfileEntry extends AlwaysSelectedEntryListWidget.Entry<ProfileEntry> {

    private static final Identifier ICON_OVERLAY_LOCATION = new Identifier("textures/gui/resource_packs.png");
    ...
    public void render(MatrixStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks) {
        ...
        MinecraftClient.getInstance().textRenderer.drawWithShadow(pMatrixStack, name, pLeft, pTop + 2, 0xFFFFFF);
        if (this.profileList.getSelectedOrNull() == this) {
    ...
    private static class ArrowButton {
        ...
        public void render(MatrixStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks) {
            ...
            if (isMouseOver(pMouseX, pMouseY)) {
                DrawableHelper.drawTexture(pMatrixStack, x, y, textureX, textureY + 32, width, height, 256, 256);
            } else {
                DrawableHelper.drawTexture(pMatrixStack, x, y, textureX, textureY, width, height, 256, 256);
            }
        }
    }
}
